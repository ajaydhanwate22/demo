...
const DailyTaskSelf = ({ navigation }) => {
  ...
  const [DailyWorkingTask, setDailyWorkingTask] = useState('');
  ...
  const handleToSubmitDailyWork = async () => {
    try {
      ...
      formData.append('date', formatForApi(startDate));
      formData.append('DailyWorkingTask', DailyWorkingTask);
      formData.append('LoginId', LoginId);
      formData.append('LoginRole', LoginRole);
      formData.append('LoginRole', LoginRole);

      const response = await axios.post('https://realrate.store/AkshayUrjaSolar/Daily_Task_Self_Added_Api.php', formData, {
      ...
      if (response.data.errmessage) {
        ...
        DailyTaskListFetch();
        hideStartModal()
        Alert.alert("Message", `Your Daily Working Added`);
  ...
  const DailyTaskListFetch = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`https://realrate.store/AkshayUrjaSolar/Daily_self_task_fetch_Api.php?loginID=${LoginId}&loginRole=${LoginRole}`);
      ...
      <Pressable style={{ width: "100%", height: "90%", marginBottom: 25 }}>
        <TouchableOpacity style={{ width: "100%", height: 40, overflow: "hidden", alignSelf: "center" }}>
          <LinearGradient colors={['#ffde59', '#ff914d']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={{ width: "100%", height: "100%", display: "flex", flexDirection: "row", justifyContent: "space-evenly", alignItems: "center" }} >
            <Text><Ionicons name='document-text-sharp' size={25} color='black' /></Text>
            <Text style={{ fontSize: 19.1, color: "#000", fontWeight: "bold" }}>Day Book</Text>
            <Text style={{ fontSize: 19.1, color: "#000", fontWeight: "600" }}>{DayWorkCount}</Text>
          </LinearGradient>
        </TouchableOpacity>
        ...
        <TouchableOpacity onPress={showStartModal} style={{ width: "90%", alignSelf: "center", display: "flex", flexDirection: "row", height: 30, alignItems: "center", marginBottom: 20, marginTop: 15 }}>
          ...
          <Text style={{ fontSize: 13, color: "#000", fontWeight: "600", marginLeft: 7 }}>Add Day Book</Text>
        </TouchableOpacity>
            ...
            {loading ? (
              ...
              DailyTaskList.map((item, index) => (
                <View style={{ width: "100%", borderBottomWidth: 1, borderColor: "#000000", padding: 5, display: "flex", flexDirection: "row", flexWrap: "wrap", alignSelf: "center" }}>
                         <Text key={index} style={{ fontSize: 14, fontWeight: "bold", color: "#000", lineHeight: 30, width: "100%" }}>{item.dailyworkingdesc}</Text>
                         <Text key={index} style={{ fontSize: 12, fontWeight: "500", color: "#f01a05", width: "50%" }}>{formatDates(item.dwt_added)}</Text>
                         <Text key={index} style={{ fontSize: 12, fontWeight: "500", color: "#f01a05", width: "50%", textAlign: "center" }}>{formatTime(item.dwt_time)}</Text>
                </View>
              )
              )
            ) : (
        ...
        <Modal visible={StartVisible} style={{ width: "100%", height: "100%", backgroundColor: "#000" }}>
          ...
          <Pressable style={{ width: "90%", height: 100, alignSelf: "center", display: "flex", flexDirection: "row" }}>
            <View style={{ width: "80%", height: "100%", justifyContent: "center", }}>
              <Text style={{ fontSize: 18, color: "#f01a05", fontWeight: "bold" }}>Add  Day Book </Text>
            </View>
          ...
          <View style={{ width: "90%", alignSelf: "center" }}>
            ...
            <TextInput placeholder='Day Work' placeholderTextColor='#000' multiline={true} onChangeText={(text) => setDailyWorkingTask(text)} value={DailyWorkingTask} style={{ backgroundColor: "#fff", borderWidth: 1, width: "100%", borderRadius: 5, marginTop: 5, marginBottom: 5, paddingLeft: 15, height: 130 }} />
